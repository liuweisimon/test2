...
BOSH 命令行界面是用户使用终端会话与 BOSH 进行交互的途径。
...
运行状况监视器对系统中的事件具有简单的感知能力，因此如果是更新组件，它便不会发出提醒。
...
可以在一个空的 git repo 中使用 `bosh init release command` 来进行初次发行版配置。
...
red;">红色</span>显示的文本需替换成您的具体信息，如 Amazon 访问密钥 ID 和 bucket 名称。
